...
def do_jaccard(query):
    search_results = Game.find_by_name(query)
    if len(search_results) > 0:
        # Let's just look at the first one for now
        query_game = search_results[0]
        tag_set = set(query_game.tags.keys())
        matching_app_ids = Tag.get_games_with_tags(tag_set)
        to_get = set()
        for games in matching_app_ids.values():
            to_get.update(games)
        to_get.remove(query_game.app_id)
        ...
        logging.info("Could not find " + query)
